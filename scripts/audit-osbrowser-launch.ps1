# Phase 0 Audit - Launch OS Browser with remote debugging, run CDP audit, stop browser

$ErrorActionPreference = 'Continue'
$port = 9223
$exe = './out/win-unpacked/OS Browser.exe'
$profileDir = Join-Path $env:TEMP 'os-browser-audit-profile2'
...
if (-not $ready) {
  Write-Host 'ERROR: Could not connect to OS Browser remote debugging port.' -ForegroundColor Red
  if ($proc -and -not $proc.HasExited) { Stop-Process -Id $proc.Id -Force }
  exit 1
}

Write-Host 'Running CDP audit script...' -ForegroundColor Cyan
node scripts/audit-osbrowser-cdp.js
